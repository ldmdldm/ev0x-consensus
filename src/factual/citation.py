...
class CitationVerifier:
    ...
    def verify_claim(self,
                     claim: str,
                     source_types: Optional[Union[SourceType,
                                                  List[SourceType]]] = None) -> Tuple[bool,
                                                                                      List[Citation]]:
        ...
        if all_citations:
            ...
            if has_technical_terms:
                ...
                elif (len(all_citations) >= 2 and
                      all_citations[0].relevance_score > 0.37 and
                      all_citations[1].relevance_score > 0.32):
    ...
    def _search_arxiv(self, query: str, max_results: int = 5) -> List[Citation]:
        ...
        term_patterns = [
            ...
            r'computer vision|NLP|natural language processing|data mining|information retrieval)\b',
            # Mathematical concepts - expanded
            r'\b(?:function|equation|theorem|probability|statistical|distribution|'
            r'optimization|bayesian|linear algebra|vector|matrix|tensor)\b',
            # Research elements
            r'\b(?:experiment|result|evaluation|analysis|assessment|evidence|'
            r'validation|measurement|benchmark|baseline|ablation study)\b',
        ]
        ...
        specific_technical_terms = [
            ...
            r'\b(?:deep reinforcement learning|convolutional neural network|transformer model|'
            r'attention mechanism|generative adversarial network|large language model|LLM|GPT|ResNet|'
            r'BERT|NLP|T5|RoBERTa|DeBERTa|DistilBERT|XLNet|ALBERT|ELECTRA|CLIP|DALL-E|'
            r'Stable Diffusion|diffusion model)\b',
            # Graph and knowledge terms
            r'\b(?:knowledge graph|semantic network|graph neural network|transfer learning|'
            r'semi-supervised learning|self-supervised learning|contrastive learning|'
            r'multimodal learning|few-shot learning|zero-shot learning|meta-learning|active learning)\b',
            # Architectures and components
            r'\b(?:LSTM|GRU|RNN|CNN|VAE|autoencoder|transformer|encoder-decoder|'
            r'embedding|fine-tuning|pre-training|tokenization|self-attention|'
            r'cross-attention|multi-head attention)\b'
        ]
    ...
    def _search_pubmed(self, query: str, max_results: int = 5) -> List[Citation]:
        ...
        term_patterns = [
            r'\b(?:disease|syndrome|disorder|condition|infection|virus|bacteria|pathogen|'
            r'treatment|therapy|medication|drug|diagnosis|prognosis|symptom|medicine|'
            r'vaccine|immunity|antibody|gene|protein|receptor|enzyme|cell|tissue|organ)\b',
            r'\b(?:clinical|medical|therapeutic|pharmaceutical|biological|genetic|'
            r'molecular|cellular|pathological|immunological|neurological|cardiovascular|'
            r'endocrine|gastrointestinal|respiratory|renal|hepatic|musculoskeletal|'
            r'dermatological|hematological|oncological)\b',
        ]
    ...
    def _extract_key_phrases(self, text: str) -> List[str]:
        ...
        try:
            ...
            try:
                numerical_data = re.findall(
                    r'\b\d+(?:\.\d+)?(?:\s*%|\s*percent|\s*mg|\s*kg|\s*ml|\s*g)\b', text, re.IGNORECASE)
            ...
            scientific_patterns = [
                ...
                r'\b(?:accuracy|precision|recall|f1|specificity|sensitivity|'
                r'error rate|auc|roc|map|ndcg|perplexity|bleu|rouge|meteor)\b',
                # Technical fields - expanded with more technical areas
                r'\b(?:machine learning|deep learning|neural network|artificial intelligence|'
                r'computer vision|NLP|reinforcement learning|generative AI|'
                r'large language model|foundation model|multimodal learning)\b',
                # ML/AI specific terms with high relevance for technical content
                r'\b(?:BERT|GPT|transformer|embedding|fine-tuning|pre-training|attention|'
                r'self-attention|cross-attention|head|layer|token|tokenization|'
                r'parameter|weight|bias|gradient|backpropagation)\b'
            ]
            ...
            multi_word_patterns = [
                ...
                r'\b(?:convolutional neural network|recurrent neural network|' +
                r'generative adversarial network|support vector machine|' +
                r'decision tree|random forest|gradient boosting|' +
                r'deep reinforcement learning|variational autoencoder|' +
                r'graph neural network|mixture of experts|' +
                r'encoder-decoder architecture|sequence-to-sequence model)\b',
                # Multi-word medical terms
                r'\b(?:randomized controlled trial|systematic review|' +
                r'meta analysis|clinical trial|case report|' +
                r'cohort study|double blind|placebo controlled)\b',
                # Add specific technical AI/ML terms that are commonly used in queries - expanded with versions and variants
                r'\b(?:BERT|GPT-3|GPT-4|GPT-3\.5|transformer|' +
                r'attention mechanism|ResNet|language model|neural architecture|' +
                r'self-supervised learning|masked language modeling|' +
                r'next token prediction|contrastive learning|prompt engineering|' +
                r'in-context learning|chain-of-thought|few-shot learning|zero-shot learning)\b',
                # Add specific architectures and versions
                r'\b(?:BERT-base|BERT-large|RoBERTa|T5|T5-base|T5-large|' +
                r'DeBERTa|XLNet|ALBERT|ELECTRA|DALL-E|CLIP|Llama|Llama-2|' +
                r'Mistral|Stable Diffusion|MidJourney|PaLM|Bard|Claude|Gemini)\b'
            ]
            ...
            important_words = re.findall(
                r'\b(?:significant|important|novel|effective|improved|advanced|innovative)\b',
                text,
                re.IGNORECASE)
