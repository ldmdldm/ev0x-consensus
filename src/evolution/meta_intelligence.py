...
from src.factual.citation import CitationVerifier, VerifiedOutput, SourceType, Citation
...
class MetaIntelligence:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        self.meta_models = {}  # Models that predict other models' behavior
    ...
    async def enhance_with_citations(self, model_output: str, domain: Optional[str] = None) -> VerifiedOutput:
        ...
        try:
            ...
            verified_output = await verifier.verify_output(model_output, domain)
            
            # Log verification statistics
            verified_count = sum(1 for citation in verified_output.citations if citation.verified)
            logger.info(f"Citation enhancement complete. Found {len(verified_output.citations)} factual claims, {verified_count} verified")
            
            # Log by source type if any citations exist
            if verified_output.citations:
                source_type_counts = {}
                for citation in verified_output.citations:
                    source_type = citation.source_type.value if hasattr(citation, 'source_type') else 'unknown'
                    source_type_counts[source_type] = source_type_counts.get(source_type, 0) + 1
                
                logger.info(f"Citation sources: {source_type_counts}")
...
async def add_citations_to_output(output: str, domain: Optional[str] = None) -> VerifiedOutput:
    ...
    try:
        ...
        # Use the verifier to verify the output
        verified_output = await verifier.verify_output(output, domain)
        
        # Log verification statistics
        verified_count = sum(1 for citation in verified_output.citations if citation.verified)
        logger.info(f"Citation verification complete. Found {len(verified_output.citations)} factual claims, {verified_count} verified")
